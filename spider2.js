...
// 1. 请求目标网站
const url = `https://voice.baidu.com/act/newpneumonia/newpneumonia?city=%E6%B9%96%E5%8C%97-%E6%B9%96%E5%8C%97`
superagent
  .get(url)
  .then(res => {
...
//    var $getStatisticsService = $('#getStatisticsService').html()
    ...
    // 使用字符切割 正则匹配 eval函数
   // var $provincetrendlist=$('provincetrend.*').html()
   var str=$('#captain-config').html()
  var dataObj= str
    ...
    //eval($getStatisticsService.replace(/window/g, 'dataObj'))
     console.log(str)
    // 3. fs写入数据到本地
    fs.writeFile(path.join(__dirname, './data2.json'), JSON.stringify(dataObj), err => {
      if (err) throw err
      console.log('数据写入成功')
    })
  })
